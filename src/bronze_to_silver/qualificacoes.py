...
def transform(df):
    try:
        ...
        columns = ["cod_qualificacoes", "descricao"]

        df = df.toDF(*columns) \
            .dropDuplicates(subset = ['cod_qualificacoes']) \
...
def main():
    try:
        ...
        df = extract("s3a://empresas-brasil/bronze/qualificacoes.parquet")
        df = transform(df)
        load(df, "qualificacoes.parquet")
