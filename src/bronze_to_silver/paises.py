...
def transform(df):
    try:
        ...
        columns = ["cod_pais", "descricao"]

        df = df.toDF(*columns) \
            .dropDuplicates(subset = ['cod_pais']) 
...
def main():
    try:
        ...
        df = extract("s3a://empresas-brasil/bronze/paises.parquet")
        df = transform(df)
        load(df, "paises.parquet")
