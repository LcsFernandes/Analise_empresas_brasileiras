...
spark = SparkSession \
        .builder \
        .appName("tabela_natureza_juridica") \
...
def transform(df):
    try:
        ...
        columns = ["cod_natureza_juridica", "descricao"]

        df = df.toDF(*columns) \
            .dropDuplicates(subset = ['cod_natureza_juridica']) 
...
def main():
    try:
        ...
        df = extract("s3a://empresas-brasil/bronze/natureza_juridica.parquet")
        df = transform(df)
        load(df, "natureza_juridica.parquet")
