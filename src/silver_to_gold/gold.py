...
def total_estabelecimentos_por_st_cadastral(df_estabelecimentos):
    try:
        ...
        df_total_estabelecimentos_por_st_cadastral = df_estabelecimentos.groupBy("st_cadastral", "descricao_st_cadastral", "cod_municipio") \
                                                                        .count() \
...
def principais_atividades_economicas(df_estabelecimentos, df_cnaes):
    try:
        ...
        df_principais_atividades_economicas = df_estabelecimentos.join(df_cnaes, on = df_estabelecimentos.cod_cnaes == df_cnaes.cod_cnaes) \
                    .groupBy(df_estabelecimentos.cod_cnaes, df_estabelecimentos.cod_municipio, df_cnaes.descricao) \
                    .count() \
                    .withColumnRenamed("count", "Total") \
                    .orderBy(desc("Total"))
...
def load(df, filename):
    try:
        path = f"s3://empresas-brasil/gold/{filename}"
